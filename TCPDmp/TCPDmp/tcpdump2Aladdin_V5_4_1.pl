...
#	my $SIMPLE_length_in_char;
	...
	foreach my $num  (0..$#Indexes){
		...
		if( hex($ether_type) == 0x800) {		
			...
			print FoutLog "$msecs;"if($debug == 3);
			$msecs = Conversion::micro2Milli($msecs);
			print FoutLog "$msecs\n"if($debug == 3);
			# suppression de la date 
			$secs = ($secs % (24*3600));
			@time = (0,0,$secs,$msecs);
			print FoutLog " Heure : $secs; $msecs;" if($debug == 3);
...
# Traitement des flux XDH
				...
				if(length($XDH_msg) == ($XDH_length*2)+4){
					...
					print FoutLog "$chrono; $heure; $minute; $seconde; $milli\n" if ($debug == 3);
...
sub conv2Time {
	...
	print FoutLog "$heure $minute $seconde $milli\n" if($debug == 3);
	$milli = int((($seconde-int($seconde))*1000)+ 0.5);
	print FoutLog "$heure $minute $seconde $milli\n" if($debug == 3);
	$seconde = int( $seconde);
	#print FoutLog "$heure $minute $seconde $milli\n";
