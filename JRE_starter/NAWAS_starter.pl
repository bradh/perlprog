...
sub startDLIP {
	my $image = $startButton[$DLIP_NUM]->cget('-image');
	print "$image\n";
	;
	...
	if (isSolarisProcessRunning ($DLIP_EXE) ){
		$stopButton[$DLIP_NUM]->configure(-image => 'stopInactivePhoto');
		$startButton[$DLIP_NUM]->configure(-image => 'startPhoto');
	}
	else {
		$stopButton[$DLIP_NUM]->configure(-image => 'stopPhoto');
		$startButton[$DLIP_NUM]->configure(-image => 'startInactivePhoto');
	}
}

sub stopDLIP {
	my $image = $stopButton[$DLIP_NUM]->cget('-image');
	print "$image\n";
