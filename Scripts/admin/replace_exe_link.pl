...
if( ! $opt_h && $opt_e ) {
	if( -f $New_link){
		...
		if ($Exe_path =~ /\/ATR\//){
			...
			print "ln -s /h7_usr/sil2_usr/samptivq/tools/Host_test_Driver/host_test_driver $Exe_path\n";
			system("ln -s /h7_usr/sil2_usr/samptivq/tools/Host_test_Driver/host_test_driver $Exe_path\n");
		}
	}
}
exit 0;
