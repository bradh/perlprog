package aladdin_FIM01_J3_6;

my $test = 0;

my $FIM01_J3_6 = "00:00:00.000 00000016 06000001 0000 0348 0000 0000 0000 0000 0007 0015 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000";
...
sub format_FIM01_J3_6{
	my $r_FIM01_J3_6 = shift;
	if($$r_FIM01_J3_6 =~ /^(\d{2}:\d{2}:\d{2}\.\d{3}) (\w{8}) (\w{8}) (.*$)/){
		my $time = $1;
		my $length = $2;
		my $msg_ID = $3;
		my $msg_data = $4;
		$msg_data =~ s/(\S{4})/$1 /g;
		$$r_FIM01_J3_6 = "$time $length $msg_ID $msg_data";
	}
}

sub get_FIM01_J3_6{
	my $r_FIM01_J3_6 = shift;
	my $message = "$r_FIM01_J3_6->{time} $r_FIM01_J3_6->{length} $r_FIM01_J3_6->{msg_id} $r_FIM01_J3_6->{msg_data}";
	print "$message\n";
	return $message;
}
...
sub setTimetag_Message{
	...
	return -1 if(!defined($r_FIM01_J3_6));
	if($$r_FIM01_J3_6 =~ /^(\d{2}:\d{2}:\d{2}\.\d{3}) (\w{8}) (\w{8}) (.*$)/){
		#print "$timetag_msg, $$r_FIM01_J3_6\n";
		my $time = $1;
		my $length = $2;
		my $msg_ID = $3;
		my $msg_data = $4;
...
sub setSTN {
	...
	#print "$STN_binary\n";
	return -1 if(! defined($r_FIM01_J3_6));
	my $binary_msg = toBinaryDataMsg($r_FIM01_J3_6, $r_FIM01_J3_6->{'msg_data'});
	#print "$binary_msg\n";
	substr($binary_msg, $STN_offset, $STN_length, $STN_binary);
	#print "$binary_msg\n";
	my $hexa_msg = binaryToHexaDataMsg($r_FIM01_J3_6, \$binary_msg);
	#print "$hexa_msg\n";
	$hexa_msg =~ s/(\S{4})/$1 /g;
	$r_FIM01_J3_6->{'msg_data'} = $hexa_msg;
	#print "$hexa_msg\n";
	return $hexa_msg;
}
...
sub addTime(){
	my $r_FIM01_J3_6 = shift;
	my $time = shift;
	$r_FIM01_J3_6->{time} =~ s/^(\d{2}:\d{2}:\d{2}\.\d{3})/$time/;
	#print "+time $$r_FIM01_J3_6\n";
}
...
if($test == 1){
	...
	open Fout, ">test.xhd";
	for my $i (0..90){
		$FIM01_J3_6 = $FIM01_J3_6_ref;
		setLatitude(Conversion::latitude2BAM_32($i+0.5), \$FIM01_J3_6);
		setLongitude(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_6);
		setCourse(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_6);
		#print Fout "$FIM01_J3_6\n";		
	}
	for my $i (90..180){
		$FIM01_J3_6 = $FIM01_J3_6_ref;
		setLatitude(Conversion::latitude2BAM_32(90), \$FIM01_J3_6);
		setLongitude(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_6);
		setCourse(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_6);
		#print Fout "$FIM01_J3_6\n";		
	}
	for my $i (0..90){
		$FIM01_J3_6 = $FIM01_J3_6_ref;
		setLatitude(Conversion::latitude2BAM_32(-90+0.5), \$FIM01_J3_6);
		setLongitude(Conversion::longitude2BAM_32(-180+$i+0.5), \$FIM01_J3_6);
		setCourse(Conversion::longitude2BAM_32(-180+$i+0.5), \$FIM01_J3_6);
		#print Fout "$FIM01_J3_6\n";		
	}
	for my $i (90..180){
		$FIM01_J3_6 = $FIM01_J3_6_ref;
