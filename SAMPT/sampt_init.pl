...
sub link_exe() {
  for $FILE (`ls  $REP_ENV/`) {
    chomp $FILE;
	# Config C2
	if (-f "$REP_ENV/$FILE" && $opt_c < 4){
	   print "Copie de $REP_ENV/$FILE vers root\@$DLIP_NAME:/rd1/RUN\n";
		system( "$RSH_CMD rm -fr /rd1/RUN/$FILE");
	   system ("rcp $REP_ENV/$FILE root\@$DLIP_NAME:/rd1/RUN");
	}
	# Config NONC2
	if (-f "$REP_ENV/$FILE" && $opt_c > 3){
	   print "Copie de $REP_ENV/$FILE vers root\@$DLIP_NAME:/rd1/RUN\n";
		system( "$RSH_CMD rm -fr /rd1/RUN/$FILE");
	   system ("rcp $REP_ENV/$FILE root\@$DLIP_NAME:/rd1/RUN");
         if($FILE =~ /init_GF/){
           print "Copie de $REP_ENV/$FILE vers root\@$DLIP_NAME:/hd1/shared_space\n";
		system( "$RSH_CMD rm -fr /rd1/RUN/$FILE");
           system ("rcp $REP_ENV/$FILE root\@$DLIP_NAME:/hd1/shared_space");
         }
	}
  }     
  return 0;
