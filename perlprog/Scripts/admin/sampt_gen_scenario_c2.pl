...
my $REP_REF_ATP = "$REP_TEST/reference_ATP_V3";
...
if ($opt_h) { 
  print "sampt_gen_scenario_c2.pl [-h] [-s][-u][c]: génère les scénarios UMAT et SIMPLE communs  \n";
}
...
if( ! $opt_h && ! $opt_t) {
  ...
  if($opt_u){
	...
	(! -f "$REP_TEST_RUN_UMAT/$FILE")or die "$REP_TEST_RUN_UMAT/$FILE existe : supprimer le et relancer le script !\n";
	  ...
	  #system ("rm -f $REP_TEST_RUN_UMAT/$FILE");
	  #print "rm -f $REP_TEST_RUN_UMAT/$FILE\n";
...
# Traitement des tests SIMPLE
  if($opt_s){
	...
	if($FILE =~ /\.conf/ || $FILE =~ /\.fim/ || $FILE =~ /\.fom/ || $FILE =~ /\.cfg/|| $FILE =~ /\.xhd/){
	  #print "$FILE\n";
	  #system ("rm -f $REP_TEST_RUN_SIMPLE/$FILE");
	  #print "rm -f $REP_TEST_RUN_SIMPLE/$FILE\n";
...
# Traitement des Tests COMUNS SIMPLE/UMAT
  if($opt_c){
