...
then
	...
	GAN_VERSION=$1
	MACHINE_NAME=$2
	MACHINE_IP=`cat ~/$MACHINES_CIBLES | grep -w $MACHINE_NAME | awk '{print $2}'`
	echo $MACHINE_NAME $MACHINE_IP > ~/$SEPARATEUR$MACHINES_CIBLES
	MACHINES_CIBLES=$SEPARATEUR$MACHINES_CIBLES
	RE_DEPLOIEMENT=1
fi
...
for f in $(cat ~/$MACHINES_CIBLES  | awk '{print $2}') # On boucle sur chaque ligne trouvee dans ce fichier
do
...
MACHINE_IP=$f
MACHINE_NAME=`cat  ~/$MACHINES_CIBLES | grep -w $MACHINE_IP | awk '{print $1'}`
...
# on recupere le No de version installe sur la machine cible 
