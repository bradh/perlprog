...
( my $lb = $frame3->new_tk__text( -height => 30, -width => 100 ) )
  ->g_grid( -column => 0, -row => 0, -sticky => "nwes" );
(
...
sub startNonReg {
	...
	foreach my $r_test (@TEST_LIST) {
		...
		$result = system("plink $puttySession $REMOTE_ENV; cd $run_dir; martha_check_test.sh") if ( $type =~ /F/ && $processCheck);
		$result = system("plink $puttySession $REMOTE_ENV; cd $run_dir; martha_check_perfo.sh") if ( $type =~ /P/ && $processCheck);
		print "$result\n";
