...
# Lancement des exe sur le rack si $opt_a n'est pas comme option
	if (! $opt_a) {
		...
		system("xterm -title SAMPT_MAIN -geometry 150x40-300+300 -e $RSH_CMD 'TZ=UCT ; export TZ ;cd $REP_RUN ; sampt_main 2>&1 > /hd2/RECORDING/sampt_main.log ' & ")if($opt_d);
