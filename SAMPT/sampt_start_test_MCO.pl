...
# Definir le repertoire de test 
	...
	$REP_RUN_TEST = "$REP_TEST_CONFIG/$VERSION/$NOM_TEST";
	print "$REP_RUN_TEST\n";
...
# Définir la la durée du  test dans le fichier listant les tests à passer
	...
	if($opt_s){
		...
		open Fin, "<$REP_RUN_TEST/$NOM_TEST.conf" or die " Imposible ouvrir $REP_RUN_TEST/$NOM_TEST.conf !\n";
		while(<Fin>){
			my $line = $_;
			chomp $line;
			if($line =~/SCENARIO_TIME\s*=\s*(\d*)\./) {
				$Duree = $1 + 10;
				print "Duree lue + 10 = $Duree\n";
			}
		}
	}
...
# Lancement des exe sur le rack si $opt_a n'est pas comme option
	if (! $opt_a) {
		print "Arrêt des exe Lynx ...\n";
		system("$RSH_CMD /rd1/LAUNCHER/kill_tasks.sh");
		sleep 1;
		#print "Lancement recorder...\n";
		system("xterm -title RECORDER -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ; recorder recorder.conf 2>&1 | tee /hd1/ivq/recorder.log ' &") if(! $opt_d && ! $opt_x);
		system("xterm -title RECORDER -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ; recorder recorder.conf 2>&1 | tee /hd1/ivq/recorder.log ' &") if(  $opt_d && ! $opt_x);
		system("$RSH_CMD 'cd $REP_RUN ; recorder recorder.conf' &")if($opt_x);
		print "Lancement slp...\n";
		system("xterm -title SERVER_SLP -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ;slp 5001 2>&1 | tee /hd1/ivq/slp.log ' &") if (! $opt_d && ! $opt_x);
		system("xterm -title SERVER_SLP -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ;slp 5001 2>&1 | tee /hd1/ivq/slp.log ' &") if (  $opt_d && ! $opt_x);
		system("$RSH_CMD 'cd $REP_RUN ;slp 5001' &") if ($opt_x);
		...
		system("xterm -title SAMPT_MAIN -geometry 150x40-300+300 -e $RSH_CMD 'TZ=UCT ; export TZ ;cd $REP_RUN ; sampt_main 2>&1 | tee /hd1/ivq/sampt_main.log ' &") if(! $opt_d  && ! $opt_x);
		system("xterm -title SAMPT_MAIN -geometry 150x40-300+300 -e $RSH_CMD 'TZ=UCT ; export TZ ;cd $REP_RUN ; sampt_main 2>&1 | tee /hd1/ivq/sampt_main.log ' &") if(  $opt_d && ! $opt_x);
		system("$RSH_CMD 'TZ=UCT ; export TZ ; cd $REP_RUN ; sampt_main ' & ")if($opt_x);
		print "Lancement SNCP pour rien...\n";
		system("xterm -title SNCP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; SNCP -debug SNCP.xml 2>&1 | tee /hd1/ivq/sncp.log '&") if(! $opt_d && ! $opt_x);
		system("xterm -title SNCP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; SNCP -debug SNCP.xml 2>&1 | tee /hd1/ivq/sncp.log '&") if(  $opt_d && ! $opt_x);
		system("$RSH_CMD 'cd $REP_RUN ; SNCP SNCP.xml'&") if($opt_x);
		print "Lancement SUIP...\n";
		system("xterm -title SUIP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; suip_main    2>&1 | tee /hd1/ivq/suip.log '&") if(! $opt_d && ! $opt_x);
		system("xterm -title SUIP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; suip_main -d 2>&1 | tee /hd1/ivq/suip.log '&") if(  $opt_d && ! $opt_x);
	}
# Lancement des TD	
	synchroniser();
	print "Lancement TD...\n";
	...
	system("xterm -title L16_TEST_DRIVER -geometry 150x40-300+300 -e /h7_usr/sil2_usr/samptivq/tools/L16_Test_Driver/l16_test_driver l16_test_driver.conf $year $month $day $second & ")if($opt_l && !$opt_x);
	system("/h7_usr/sil2_usr/samptivq/tools/L16_Test_Driver/l16_test_driver l16_test_driver.conf  $year $month $day $second &")if($opt_l && $opt_x);
	...
	system("xterm -title TI_TEST_DRIVER -geometry 150x40-300+300 -e /h7_usr/sil2_usr/samptivq/tools/L16_Test_Driver/ti_test_driver ti_test_driver.conf  $year $month $day $second &")if($opt_i && !$opt_x);
	system(" /h7_usr/sil2_usr/samptivq/tools/L16_Test_Driver/ti_test_driver ti_test_driver.conf $year $month $day $second &2>&1 > /dev/null")if($opt_i && $opt_x);
	...
	system("xterm -title HOST_TEST_DRIVER -geometry 150x40-300+300 -e /h7_usr/sil2_usr/samptivq/tools/Host_test_Driver/host_test_driver host_test_driver.conf $year $month $day $second &")if(!$opt_x);
	system("/h7_usr/sil2_usr/samptivq/tools/Host_test_Driver/host_test_driver host_test_driver.conf  $year $month $day $second &")if($opt_x);
...
# Recuperation des log du rack Lynx
	system("sampt_retrieve_log_MCO.pl -r $opt_r -v SAMPT_MCO -t $opt_t -c $opt_c");
# Suppression des fim technique
	system("remove_fxm_technique.pl -f $NOM_TEST.fim");
} 
exit 0;
