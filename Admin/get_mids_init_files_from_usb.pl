...
# Recuperation du fichier mission.xml
die "XML mission file $XML_MISSION_FILE_NAME not found\n" if( ! -d  $XML_MISSION_TARGET_DIR
																|| ! -e $XML_MISSION_TARGET_DIR/$XML_MISSION_FILE_NAME);
# Liste des fichiers d'init MIDS present sur la cible
my @mids_init_files;
...
open Fout, "> $XML_MISSION_TARGET_DIR/$XML_MISSION_FILE_NAME.bak" or die "file $XML_MISSION_TARGET_DIR/$XML_MISSION_FILE_NAME.bak not found\n";
while(<Fin>){
	my $line = $_;
	...
	if ($line !~ /<RESOURCE ID/){
		...
		foreach my $index (0.. $#mids_init_files){
			my $j= $index + 1;
			print Fout "\t\t\<FILE ID=\"$j\" LABEL=\"INIT 1\" FILE=\"$mids_init_files[$index]\" \/\>";
		}
	}
}
close Fin;
