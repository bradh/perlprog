...
if ($opt_h) { 
	print "sampt_start_test.pl [-h] [-c 1 à 9] [-v nom_version][-t nom_test]: init du repertoire de run  \n";
...
if ($CONFIG_TEST == 1 || $CONFIG_TEST == 2 || $CONFIG_TEST ==  4 || $CONFIG_TEST == 5 ) {
	...
	system("dtterm -title RECORDER -geometry 68x20-0+0 -e $RSH_CMD 'cd $REP_RUN ; recorder recorder.conf '&");
	system("dtterm -title SERVER_SLP -e $RSH_CMD 'cd $REP_RUN ;slp 5001 '&") ;
	synchroniser();
	system("dtterm -title SAMPT_MAIN -geometry 100x40-300+300 -e $RSH_CMD 'cd $REP_RUN ; sampt_main '&");
	system("dtterm -title L16_TEST_DRIVER -e l16_test_driver l16_test_driver.conf $year $month $day $second &")if($opt_l);	
#	system("dtterm -title L16_TEST_DRIVER -e l16_test_driver l16_test_driver.conf  $year $month $day $second &")if($opt_l);	
	system("dtterm -title TI_TEST_DRIVER -e l16_test_driver ti_test_driver_c2.conf $year $month $day $second &")if($opt_l);
	system("dtterm -title HOST_TEST_DRIVER -e ./host_test_driver t_*.conf $year $month $day $second &");
#	system("$RSH_CMD 'ps-ax -r 10 >> RAM_utile.txt'&")if($opt_l);
	system("watchCPU.pl -r $opt_r -c $CONFIG_TEST -v $VERSION_DLIP -t $opt_t &")if($opt_l);
	system("watchRAM.pl -r $opt_r -c $CONFIG_TEST -v $VERSION_DLIP -t $opt_t &")if($opt_l);
	sleep 2;
	creer_stop_file();
}
...
#	system("dtterm -title SNCP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; SNCP -debug > /home/run/sncp.log 2>&1 '&");	
	system("dtterm -title RECORDER -geometry 68x20-0+0 -e $RSH_CMD 'cd $REP_RUN ; recorder recorder.conf '&");
	system("dtterm -title HOST_TEST_DRIVER -geometry 100x20-300+0 -e ./host_test_driver t_*.conf &");
	system("watchCPU.pl -r $opt_r -c $CONFIG_TEST -v $VERSION_DLIP -t $opt_t &")if($opt_l);
	sleep 2;
	creer_stop_file();
}
