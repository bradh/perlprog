...
my $DLIPMng_RUN_DIR = "E:\\THALES\\DLIPmng_V1R1E5_XP10\\DLIPL16Mng-Delivery";

my $JREM_EXE = "launch.bat";
my $JREM_WIN_NAME = "JRE Management";
my $JREM_RUN_DIR = "E:\\THALES\\JREM_V14R3_XP10\\bin";
...
sub startDLTE_ALTBMD {
	...
	if( $image eq "startInactivePhoto" 
		...
		&& confirmAction ("Start DLTE JRE ?") eq "Yes") {
		print "$image\n";
...
sub stopDLTE_ALTBMD {
	...
	#print "$image\n";
	if( $image eq "stopInactivePhoto" && isWindowsProcessRunning ($DLTE_ALTBMD_WIN_NAME) && confirmAction ("Stop DLTE JRE ?") eq "Yes") {
		print "$image\n";
