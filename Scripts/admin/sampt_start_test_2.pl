...
# Lancement des exe sur le rack si $opt_a n'est pas comme option
	if (! $opt_a) {
		...
		system("xterm -title SAMPT_MAIN -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ; sampt_main ' &")if(! $opt_d );
		system("xterm -title SAMPT_MAIN -geometry 150x40-300+300 -e $RSH_CMD 'cd $REP_RUN ; sampt_main 2>&1 > /hd2/RECORDING/sampt_main.log ' & ")if($opt_d);
		#system("$RSH_CMD 'cd $REP_RUN ; sampt_main ' & ")if($opt_d);
		print "Lancement SNCP pour rien...\n";
		system("xterm -title SNCP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; SNCP SNCP.xml'&") if(! $opt_d);
		system("xterm -title SNCP -geometry 68x34-0-300 -e $RSH_CMD 'cd $REP_RUN ; SNCP  SNCP.xml 2>&1 > /hd2/RECORDING/sncp.log '&") if($opt_d);
...
# Lancement des TD	
	...
	system("xterm -title TI_TEST_DRIVER -geometry 150x40-300+300 -e ./l16_test_driver ti_test_driver.conf $year $month $day $second &")if($opt_i);
	...
	system("xterm -title HOST_TEST_DRIVER -geometry 150x40-300+300 -e ./host_test_driver host_test_driver.conf $year $month $day $second &");
...
#	system ("$RSH_CMD 'cd $REP_RUN ; rm slp.log'");
	if (! $opt_a){
		...
		sleep 1;
		#print "Lancement recorder...\n";
		#system("xterm -title RECORDER -geometry 68x20-0+0 -e $RSH_CMD 'cd $REP_RUN ; recorder recorder.conf'&")if(! $opt_d);	
...
# Lancement des TD	
	...
	system("xterm -title TI_TEST_DRIVER -e ./ti_test_driver ti_test_driver_nonc2.conf $year $month $day $second &")if($opt_i);
# Lancement des scripts de mesure
	#system("$RSH_CMD '/hd2/SCRIPTS_IVQ/watchCPU.sh' &")if($opt_w);
	system("watchCPU.pl -r $opt_r -c $CONFIG_TEST -v $VERSION_DLIP -t $opt_t &")if($opt_w);	
	system("watchRAM.pl -r $opt_r -c $CONFIG_TEST -v $VERSION_DLIP -t $opt_t &")if($opt_w);
