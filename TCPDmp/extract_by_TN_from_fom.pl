...
my $Offset_motI=17; # offset du mot I dans le tableau @MOT $MOT[11] est le 1er boctet
sub getJValue {
...
if( ! $opt_h ) {
	...
	while(<>){
		...
		if($Label == 2) {
			...
			$fichierOutput = "J$Label-$Sublabel-STN_$STN.fim";
...
# Description :
# extrait de l'entrée standard les messages J3.X J2.X avec leur TN / Source TN 
...
if( ! $opt_h ) {
	...
	while(<>){
		...
		if($Label == 2) {
			my $STN = hex("$MOT[4]");
			print "$Label $Sublabel STN : $STN\n";
			$fichierOutput = "J$Label-$Sublabel-STN_$STN.fom";
			#."-"."$Sublabel"."-STN_"."$STN"."fom";
			print "$fichierOutput\n";
			open Fout, ">>$fichierOutput"  or open ">$fichierOutput"or die "impossible d'ouvrir le fichier de sortie $fichierOutput \n";
			print Fout "$LIGNE\n";
			close Fout;
