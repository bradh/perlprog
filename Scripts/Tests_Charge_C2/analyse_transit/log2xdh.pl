...
# Traitement des READ
	...
	if($findr == 0 && $line =~/READ_DONE\s+in CSC_RTE_FULL_SOCKET_DUMP\/SOCKET \( 14\)\s+(..)\s+(..)/){
	...
	if($findr == 1 && $length_xhd > 0 && $line =~/READ_DONE\s+in CSC_RTE_FULL_SOCKET_DUMP\/SOCKET \( 14\)\s+(.*)/){
			$temp_xhd_id = $1;
			#suppression des espaces
			$temp_xhd_id =~ s/\s//g;
	...
	if($findr == 2 && $length_xhd > 0 && $line =~/READ_DONE\s+in CSC_RTE_FULL_SOCKET_DUMP\/SOCKET \( 14\)\s+(.*)/){
			$temp_xhd = $1;
			#suppression des espaces
			$temp_xhd =~ s/\s//g;
			#print "$temp_xhd\n";
