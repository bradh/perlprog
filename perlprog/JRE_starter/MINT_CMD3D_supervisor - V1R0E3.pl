 ...
 # mise  hauteur des menus Configuration, Recording  et Log
 # V1R0E1 mise  hauteur du schema general
 # V1R0E0 CMD3D
 # V1R2E2 JFACC
 # ajout d'une variable $Disk_Label
 # supression de l'adresse Mids local
 # suppression du fichier contextDsata lors de l'arrt des applications
 # V1R2E1
 # suppression du fichier DLIP_Context  l'arrt du TDL router pour un redmarrage correct du ToplinkCom
...
# Simplifaction de la commande de dtection de dconnexion du MIDS
# La pop-up demandant  l'oprateur si le MIDS est utilis n'est demand qu'audmarrage et au restart
...
# Gestion des modes master et use (avec ou sans MIDS) au re demarrage du TDL router
# Gestion dynamique du fichier Context
# Suppression des menus ComMgr
# Mise au propre des menu Log
#  par S. Mouchot le  11/03/14
# Classement des RCD par repertoire date par MD et JYI le 09/04/2014
...
# Supervision cnx SIMPLE AT
my $simple_at_remote_ip = "200.1.18.52";
my $simple_at_port_num = "6005";
...
local $TDL_ROUTER = {	
				...
				'LOG_DIR' 	=> "$HOME_DIR/log/TDL_ROUTER",
...
if ($opt_h) { 
	print  "Lancement station JRE L16 de faon graphique\n";
	exit(0);
}
...
# Row 1 PAVE
	...
	# Pave TI
	$column += 1; # column 3
	$startStopButton[$TI_NUM] = $HframeSchema[$row][$column]->new_label( -text => "TI", -width => $width_pave, -height => $height_pave, -anchor => 'center', -background => 'grey');
...
# Row 4	
	...
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx, -background => 'white', -height => $height_cnx) ->g_grid(-row => $row, -column => $column);
	$column += 1; # column 3
	$SIMPLE_AT_CNX_LABEL = $HframeSchema[$row][$column]->new_label( -borderwidth => 0,  -image => $traitSimpleVerticalKOPhoto);
	$SIMPLE_AT_CNX_LABEL->g_grid(-row => $row, -column => $column);
	$column += 1; # column 4
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx,  -background => 'white', -height => $height_cnx, -anchor => 'center') ->g_grid(-row => $row, -column => $column);
	$column += 1; # column 5
	$HframeSchema[$row][$column]->new_label( -width => $width_pave, -height => $height_cnx, -background => 'white')->g_grid(-row => $row, -column => $column);	
	$column += 1; # column 6
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx, -background => 'white', -height => $height_cnx)->g_grid(-row => $row, -column => $column);
	...
	# SIMPLE AT PAVE
	$column += 1; # column 3
	$startStopButton[$SIMPLE_AT_NUM]=$HframeSchema[$row][$column]->new_label( -text => "SIMPLE AT", -width => $width_pave, -background => 'grey', -height => $height_pave, -anchor => 'center');
	$startStopButton[$SIMPLE_AT_NUM]->g_grid(-row => $row, -column => $column);
	$column += 1; # column 4
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx, -background => 'white', -height => $height_pave) ->g_grid(-row => $row, -column => $column);
	$column += 1; # column 5
	$HframeSchema[$row][$column]->new_label( -width => $width_pave,	-background => 'white', -height => $height_pave)->g_grid(-row => $row, -column => $column);
	...
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx, -background => 'white', -height => $height_pave)->g_grid(-row => $row, -column => $column);
	$column += 1; # column 9
	$startStopButton[$SIMPLE_CNX_NUM] =$HframeSchema[$row][$column]->new_label( -text => "SIMPLE NW", -width => $width_pave, -background => 'grey', -height => $height_pave, -anchor => 'center');
	$startStopButton[$SIMPLE_CNX_NUM]->g_grid(-row => $row, -column => $column);
	$column += 1; # column 10
	$HframeSchema[$row][$column]->new_label( -width => $width_cnx, -background => 'white', -height => $height_pave)->g_grid(-row => $row, -column => $column);
	$column += 1; # column 11
	$startStopButton[$JREP_CNX_NUM] =$HframeSchema[$row][$column]->new_label( -text => "JREP NW", -width => $width_pave, -background => 'grey', -height => $height_pave, -anchor => 'center');
...
#	updateContextFile();
	...
	# Si le MIDS est prsent et que la connexion au terminal est OFF et le MIDS est utilis et le TDL router tourne
...
sub updateStatusSIMPLE {	
	if( isSIMPLEPresent()) {
			$startStopButton[$SIMPLE_CNX_NUM]->configure(-background => 'green');
	}
	else {
		$startStopButton[$SIMPLE_CNX_NUM]->configure(-background => 'grey');
	}
}

sub updateStatusSIMPLEAT {	
	if( isSIMPLEATPresent()) {
			$startStopButton[$SIMPLE_AT_NUM]->configure(-background => 'green');
	}
	else {
		$startStopButton[$SIMPLE_AT_NUM]->configure(-background => 'grey');
	}
}	
...
sub isSIMPLEPresent{
	my $simple_db_present = 0;
	print "Wait for SIMPLE AT ping response ...\n" if( $debug == 2 );
	my $cmd;
	my $result;
	if($system eq "LINUX"){
		$cmd = "ping $simple_remote_ip -c 1 ";
		$cmd = "ping $simple_remote_ip " if($system eq "SOLARIS");
		$result = `plink $putty_session $cmd ` ;
		print "result = $result" if ( $debug == 2 );
		$simple_db_present = 1 if( $result !~ /0 received/);	
	}
	if($system eq "SOLARIS"){
		$cmd = "ping $simple_remote_ip 1 ";
		$result = `plink $putty_session $cmd ` ;
		print "result = $result" if ( $debug == 2 );
		$simple_db_present = 1 if( $result =~ /alive/);	
	}
	print "SIMPLE NW present  : $result : $mids_db_present\n" if($debug == 2);
	return 	$simple_db_present;
}

sub updateCnxSIMPLEAT {
	if( isSIMPLEATConnected() ) {
		$SIMPLE_AT_CNX_LABEL->configure(-image => 'trait_simple_vertical_OK');
	}
	else { 
		$SIMPLE_AT_CNX_LABEL->configure(-image => 'trait_simple_vertical_KO');
	}
}

sub isSIMPLEATPresent{
	my $simple_db_present = 0;
	print "Wait for SIMPLE AT ping response ...\n" if( $debug == 2 );
	my $cmd;
	my $result;
	if($system eq "LINUX"){
		$cmd = "ping $simple_at_remote_ip -c 1 ";
		$cmd = "ping $simple_at_remote_ip " if($system eq "SOLARIS");
		$result = `plink $putty_session $cmd ` ;
		print "result = $result" if ( $debug == 2 );
		$simple_db_present = 1 if( $result !~ /0 received/);	
	}
	if($system eq "SOLARIS"){
		$cmd = "ping $simple_at_remote_ip 1 ";
		$result = `plink $putty_session $cmd ` ;
		print "result = $result" if ( $debug == 2 );
		$simple_db_present = 1 if( $result =~ /alive/);	
	}
	print "SIMPLE AT present  : $result : $mids_db_present\n" if($debug == 2);
	return 	$simple_db_present;
}
...
sub isSIMPLEATConnected {
	my $simple_connected = 0;
	foreach my $cnx (@jre_gateway_connexion_list){
		if ( $cnx =~ /\.$simple_at_port_num/ && $cnx =~ /ESTABLISHED/){
			$simple_connected = 1;
			last;	
		}
	}
...
sub saveLog {
	if(confirmAction("Save Log File ? It will stop all applications !") eq "yes" ){
		...
		if (-d  "$LOG_DIR"){
			chdir($LOG_DIR);
			foreach my $task (@TaskList) {
				if($$task->{'OPERATING_SYST'} eq "SOLARIS" && confirmAction("Save $task log ?") eq "yes" ){
...
sub checkMIDS{
	...
	# mise  jour de la commande du TDL ROUTER fonction du MIDS
