...
# Definir le repertoire de test 
	...
	$REP_RUN_TEST = "$REP_TEST_CONFIG/$VERSION/$NOM_TEST/ATR";
	print "$REP_RUN_TEST\n";
...
# Définir la la durée du  test dans le fichier listant les tests à passer
	...
	if($opt_s){
		...
		my $CONFIG_C2_NC2 = "NON_C2";
		$CONFIG_C2_NC2 = "C2" if ($opt_c < 4);
		print "$REP_TEST\n";
  		open Fin, "<$REP_TEST/$CONFIG_C2_NC2/$REP_REF/$REP_FIC_COM/$FIC_NON_REG" or die "imposible ouvrir $REP_TEST/$CONFIG_C2_NC2/$REP_REF/$REP_FIC_COM/$FIC_NON_REG \n";
  		while (<Fin>){
    		chomp;
    		my $Ligne = $_;
    		next if( $Ligne =~ /^#/);
			my($NOM, $CONFIG_UMAT_SIMPLE, $DUREE_TEST) = split (":", $Ligne);
    		if ($NOM eq $NOM_TEST){
    			#print "$NOM, $CONFIG_UMAT_SIMPLE, $DUREE_TEST\n";
    			$Duree = $DUREE_TEST;
    			last;
    		}
  		}
...
# Recuperation des log du rack Lynx
	system("sampt_retrieve_log.pl -r $opt_r -v SAMPT_MCO -t $opt_t -c $opt_c");
} 
exit 0;
