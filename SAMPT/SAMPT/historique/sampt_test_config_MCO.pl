...
# Nettoyer le repertoire
	system("/h7_usr/sil2_usr/samptivq/tools/Scripts/admin/sampt_clean_dir.sh");
# lien vers les tests driver
	system("ln -sf /h7_usr/sil2_usr/samptivq/tools/Host_test_Driver/host_test_driver $REP_TEST_ATR/");
	system("ln -sf /h7_usr/sil2_usr/samptivq/tools/L16_Test_Driver/l16_test_driver $REP_TEST_ATR/");
# lien vers les fichiers de conf des test driver dans ATP
	system("cp -f ../ATP/host_test_driver.conf temp")if( -l "$REP_TEST_ATR/host_test_driver.conf"); 
	system("mv temp $REP_TEST_ATR/host_test_driver.conf");
	system("cp -f ../ATP/l16_test_driver.conf temp")if( -l "$REP_TEST_ATR/l16_test_driver.conf");
	system("mv temp $REP_TEST_ATR/l16_test_driver.conf");
	system("cp -f ../ATP/ti_test_driver_c2.conf temp")if( -l "$REP_TEST_ATR/ti_test_driver_c2.conf");
	system("mv temp $REP_TEST_ATR/ti_test_driver_c2.conf");
# lien vers les fichiers d'entrée
	system("cp -f ../ATP/$NOM_TEST_MINUS.xhd temp") if( -l "$REP_TEST_ATR/$NOM_TEST_MINUS.xhd");
	system("mv temp $REP_TEST_ATR/$NOM_TEST_MINUS.xhd");
	system("cp -f ../ATP/$NOM_TEST_MINUS.fom temp") if( -l "$REP_TEST_ATR/$NOM_TEST_MINUS.fom");
	system("mv temp $REP_TEST_ATR/$NOM_TEST_MINUS.fom");
	system("cp -f ../ATP/$NOM_TEST_MINUS.ji temp") if( -l "$REP_TEST_ATR/$NOM_TEST_MINUS.ji");
	system("mv temp $REP_TEST_ATR/$NOM_TEST_MINUS.ji");
	system("cp -f ../ATP/ti_test_driver_c2.dem_apu $REP_TEST_ATR/") if( -l "$REP_TEST_ATR/ti_test_driver_c2.dem_apu");
	system("mv temp $REP_TEST_ATR/ti_test_driver_c2.dem_apu");
...
print Fout "Network_Broadcast_Address = 200.1.18.2\n";
print Fout "Channel4_Hosts_Port       = 14000\n";
print Fout "Channel4_DLIP_Port        = 15000\n";
print Fout "Host_Address              = 200.1.18.50\n";
