...
sub extract_SIMPLE_Msg_Server (){
	...
	if($packetType == 1){
		...
		my $MI = decodeSIMPLEFOM($SIMPLE_msg_9); 
		print FoutLog "MI server : $MI\n" if($debug == 8);
		print FoutSIMPLEFOM "$heure:$minute:$seconde.$milli $MI \n" if($MI ne "0") ;
...
sub extract_SIMPLE_Msg_Client (){
	...
	if ($packetType == 2) {
		my $MI = decodeSIMPLEFIM($SIMPLE_msg_8); 
		print FoutLog "FIM client : $MI\n" if($debug == 10);
		print FoutSIMPLEFIM "$heure:$minute:$seconde.$milli $MI \n" if($MI ne "0") ;
