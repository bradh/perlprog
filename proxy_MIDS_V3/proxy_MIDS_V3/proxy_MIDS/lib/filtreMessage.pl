...
# Mise Ä jour le 8 octobre 2002 par S. Mouchot
...
#### Conversion du fichier finalsort.rslt au format aladdin finalsort.xdh
	...
	while (<Fin>) {
		...
		$label = toHexaString($label);		
	      $label = substr($label,-2,2);
	      my $sub_label = $tableauLigne[3];
		$sub_label = toHexaString($sub_label);		
...
sub extractAdh {
	my $fichierInput = "sampt_main.log";
