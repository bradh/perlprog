...
#        - Amélioration de la présentation des champs
#        - Add CheckButton to select interface
#        - Add Button to save configuration file
# Modif S. Mouchot le 28/06/09 anniversaire de mariage (13 ans déjà!)
...
# TCP parameters for JRE flow (server side)
my $JRE_SERVER_IP_Address = "172.25.8.170";
my $JRE_SERVER_TCP_Port = "10201";
...
if(-f "$Config_File"){
  ...
  while(<Fin>){
    ...
    $processJRE= $1 if ($line =~/processJRE= (\S*)/);
...
my $Vframe9 = $mw->Frame->pack(-pady => 1);
$Vframe9->Checkbutton( -variable => \$processJRE, -command => \&radiobuttonproceed, -width => 5)->pack(-side=>'left', -padx => '5');
$Vframe9->Label(-text => "JRE server IP Address : ", -width => 35)->pack(-side=>'left', -padx => '5');
$Vframe9->Entry(-textvariable => \$JRE_SERVER_IP_Address, -width => 15, -borderwidth => 2)->pack(-side =>'left', -padx => '5');
$Vframe9->Label(-text => 'TCP Port : ', -width => 10)->pack(-side=>'left', -padx => '5');
$Vframe9->Entry(-textvariable => \$JRE_SERVER_TCP_Port, -width => 10, -borderwidth => 2)->pack(-side =>'left', -padx => '10');


my $Vframe7_2 = $mw->Frame->pack(-pady => 1);
$Vframe7_2->Checkbutton( -variable => \$toRelative, -command => \&radiobuttonproceed, -width => 5)->pack(-side=>'left', -padx => '5');
$Vframe7_2->Label(-text => "Relative Time", -width => 35)->pack(-side=>'left', -padx => '5');

my $Vframe8 = $mw->Frame->pack;
my $ButtonSave=$Vframe8->Button(-text=>'Saving configuration', -state => 'active', -command => \&saveConfig, -width => 10)->pack(-side => 'left', -pady => 5, -padx => 5);
my $ButtonExtract=$Vframe8->Button(-text=>'Extracting', -state => 'active', -command => \&extract2Aladdin, -width => 10)->pack(-side => 'left', -pady => 5, -padx => 5);
my $ButtonFilter=$Vframe8->Button(-text=>'Filtering', -state => 'active', -command => \&filterMessage, -width => 10)->pack(-side => 'left', -pady => 5, -padx => 5);
MainLoop;
...
sub saveConfig{
    ...
    print Fout "processJRE=$processJRE\n";
...
sub extract2Aladdin {
	...
	open FoutJREout, ">$OutputDir$OutputFile.JRE.fim" or die "Impossible ouvrir JRE.fim" if($processJRE);
	open FoutJREin, ">$OutputDir$OutputFile.JRE.fom" or die "Impossible ouvrir JRE.fom" if($processJRE);
...
#	my $SIMPLEFIM_length_in_char;
	...
	if($processFIMFOM){
		print Fout "Link_Output_File_1 = $OutputDir$OutputFile.fim\n";
		print Fout "Link_Input_File_1 = $OutputDir$OutputFile.fom\n";
	...
	if($processXHDXDH){
 	...
 	if($processJRE){
		print Fout "Link_Output_File_2 = $OutputDir$OutputFile.JRE.fim\n";
		print Fout "Link_Input_File_2 = $OutputDir$OutputFile.JRE.fom\n";
		print Fout "Link_Id_1=2\n";
    	print Fout "Link_Type_2=L16\n";
    	print Fout "Link_Port_2=200\n";
    	print Fout "Link_Dictionary_File_2=C:\\Aladdin_V2\\Dictionaries\\FIM_FOM\\d_fxm_ah_l16_ed5_Altbmd1c_ML366_963.xml\n";
	}
    ...
    # Creation du fichier JRE.conf
	if($processJRE){
	                   open Fout, ">$OutputDir$OutputFile.JRE.conf" or die "Impossible ouvrir $OutputDir$OutputFile\n";
	                   print Fout "Link_Output_File_1 = $OutputDir$OutputFile.JRE.fom\n";
	                   print Fout "Link_Input_File_1 = $OutputDir$OutputFile.JRE.fim\n";
	                   close Fout;
	}
	...
	foreach my $num  (0..$#Indexes){
		...
		if( hex($ether_type) == 0x800) {		
			...
			# Le paquet TCP est un XHD
                    ...
                    # Traitement du flux JRE
                    if(($ip_dest eq $JRE_SERVER_IP_Address && $tcp_port_dest eq $JRE_SERVER_TCP_Port)) {
  						# on concatene $JREdata avec les données nouvelles reçues
  						my $tcpdatalength = length( $tcp_data);
  						$JREdata = $JREdata . unpack ( H${tcpdatalength}, $tcp_data);
  						my $JREdatalength = length( $JREdata);				
  						# on verifie si la longueur recue est au moins egale à la taille du header (18)
  						if(length($JREdata) > 18){
  							#si oui on cree un objet header
  							# my $string = substr($tcp_data, 18, 0);
  							#my $JREHeaderMessage->new(\$string);
  							# puis on calcule la longueur du message on supose que le debut de TCPdata 
  							$JREMsglength = hex(substr($string, 4, 4);
  							$JREMsf$JREHeaderMessage->messagelength();
  							# coincide avec le dé&but du message !?
  							
  							# si la longueur des données reçue est supérieur à la longueur du message
  							if($JREmessagelength < length($JREdata)){
  								# on cree le message 
  								# On traite le message : fonction appelée 
  								#On sauvegarde le reste du message
  								# ou on remet le $JREdata à vide
  							}
  							#sinon on poursuit la lecture 
  						}
  						#sinon on poursuit la lecture
  						# on 
  						my $header = unpack()
  						for (my $i=0, $i <$data_length_in_byte, $i++){
  							my ($tcp_header, $tcp_data) = unpack ("H${length_tcp_head_in_char}H${data_length_in_char}", $ip_data);
