...
# Utilisation de répertoire standart pour les applis JREM TOPLINKCOM,...
# Step 2 : ajout de la supervisio du JREP Name
# ajout de la configuration des tag des routes pour le TOPLINKCOM
# ajout de la sélection du mode useMIDS
# modifié pour ETO JREAP V3R1E0
# Suppression du rack Lynx
...
my $HOME_DIR="/home/jfacc_op";
my $HOME_WIN_DIR="D:\\THALES";
my $LOG_DIR = "/jfacclog";
...
local $TDL_ROUTER = {	
				...
				'LOG_DIR' 	=> "$HOME_DIR/Ops/TDL_ROUTER",
				'LOG_DIR_TARGET' => "$LOG_DIR/TDL_ROUTER",
...
local $JREP = {	'NUM'		=> $JREP_NUM, 
				...
				'LOG' 		=> "jre.log",
				'LOG_DIR' 	=> "$HOME_DIR/Ops/JREP/trace",
				'LOG_DIR_TARGET' => "$LOG_DIR/JREP",
...
local $HOST = {	'NUM'		=> $HOST_NUM, 
				...
				'LOG_DIR_TARGET' => "$LOG_DIR/HOST_TD",
...
sub  initSchemaButton2 {
	...
	$voyant_Status_OK = Tkx::image_create_photo("status_OK", -file => "Images/Status_OK2.gif");
	$voyant_Status_warning = Tkx::image_create_photo("status_warning", -file => "Images/Status_warning2.gif");
	$voyant_Status_alarm = Tkx::image_create_photo("status_alarm", -file => "Images/Status_alarm2.gif");
	
	
	
	my $string = " JFACC Starter ";	
	for my $i (0 ..(length($string)-1)){
		...
		if (defined($task)){
			...
			initStartStopButton2($task);
		}
		if ($i == $STATUS_NUM){
			$Hframe[$i]->new_ttk__label( -text  => "-- Status --", -width => $widthLabel, -anchor => 'w')->g_grid(-column => 1, -row => $i);
		}
		if ($i == $MIDS_CNX_NUM){
		...
		if ($i == $JREP_CNX_NUM){
			$Hframe[$i]->new_ttk__label(-text  => " JREP Cnx :", -width => $widthLabel, -anchor => 'e')->g_grid(-column => 1, -row => $i);

			$stopButton[$i] = $Hframe[$i]->new_ttk__label( -image => $voyant_Status_alarm);
			$stopButton[$i]->g_grid(-column => 4, -row => $i);
			$stopLabel[$i] = $Hframe[$i]->new_ttk__entry( -text => \$jrep_status_label, -width => $widthEntry);
		...
		if ($i == $DLP_CNX_NUM){
			...
			$stopButton[$i] = $Hframe[$i]->new_ttk__label( -image => $voyant_Status_alarm);
			$stopButton[$i]->g_grid(-column => 4, -row => $i);
			$stopLabel[$i] = $Hframe[$i]->new_ttk__entry( -text => \$dlp_status_label, -width => $widthEntry);

			$stopLabel[$i]->g_grid(-column => 5, -row => $i, -padx => 5);
		}
		if ($i == $HOST_CNX_NUM){
			$Hframe[$i]->new_ttk__label( -text  => " HOST Cnx :", -width => $widthLabel, -anchor => 'e')->g_grid(-column => 1, -row => $i);

			$stopButton[$i] = $Hframe[$i]->new_ttk__label( -image => $voyant_Status_alarm);
			$stopButton[$i]->g_grid(-column => 4, -row => $i);
			$stopLabel[$i] = $Hframe[$i]->new_ttk__entry( -text => \$host_status_label, -width => $widthEntry);
...
sub  initStartStopButton2 {
	...
	$stopButton[$j] = $Hframe[$j]->new_ttk__button(-command => [\&stopProcess2, $task],
								-image => $initialStopPhoto);
	#$stopButton[$j]->configure( -borderwidth => 15);
	$stopButton[$j]->g_grid(-column => 3, -row => $j, -sticky => 'w');
	$startButton[$j] = $Hframe[$j]->new_ttk__button(-command => [\&startProcess2, $task],
...
sub restartAllWithoutConfirmation {
	foreach my $task (@TaskList){
		if(($task eq "TDL_ROUTER") || ($task eq "TOPLINKCOM")){
...
sub updateStatusAll {
	...
	if( ( isMIDSConnected() == 0 ) 
		...
		&& confirmAction( "MIDS deconnected, TDL router restarted ..."))  {
...
sub stopProcess {
	...
	if($$task->{'OPERATING_SYST'} eq "WINDOWS"){
		...
		#if(isWindowsProcessRunning ($task) && confirmAction ("Stop $task ?") eq "yes"){
	...
	if( !$systemStart){
		if($mids_db_present ){
			if($mids_db_connected ) {
				$stopButton[$MIDS_CNX_NUM]->configure(-image => 'status_OK');
				$mids_status_label = "Connected";
			}
			else { 
				$stopButton[$MIDS_CNX_NUM]->configure(-image => 'status_warning');
				$mids_status_label = "Present";
			}
			if($first_ping_mids) {
				$first_ping_mids = 0;
				$first_disconnection_mids = 1;				
				acquittementAction("MIDS connected, restart TDL router...");
				stopProcessWithoutConfirmation("TDL_ROUTER");
			...
			$stopButton[$MIDS_CNX_NUM]->configure(-image => 'status_alarm');
			$mids_status_label = "not Present";
			if($firs_disconnection_mids){
				$fisrt_disconnexion_mids = 0;
				acquittementAction("MIDS disconnected, stopping TDL router...");
				stopProcessWithoutConfirmation("TDL_ROUTER");
				stopProcessWithoutConfirmation("DLIPCOM");
				$first_ping_mids = 1;
			}
...
sub updateCnxDLP2 {
	if( isHOSTConnected() ) {
		$stopButton[$DLP_CNX_NUM]->configure(-image => 'status_OK');
		$dlp_status_label = "Connected";
	}
	else { 
		$stopButton[$DLP_CNX_NUM]->configure(-image => 'status_alarm');
		$dlp_status_label = "not Connected";
	}
}
...
sub updateCnxJREP2 {
	if( isJREPConnected() ) {
		$stopButton[$JREP_CNX_NUM]->configure(-image => 'status_OK');
		$jrep_status_label = "Connected";
	}
	else { 
		$stopButton[$JREP_CNX_NUM]->configure(-image => 'status_alarm');
		$jrep_status_label = "not Connected";
	}
}
...
sub updateCnxHOST2 {
	if( isHOSTConnected() ) {
		$stopButton[$HOST_CNX_NUM]->configure(-image => 'status_OK');
		$host_status_label = "Connected";
	}
	else { 
		$stopButton[$HOST_CNX_NUM]->configure(-image => 'status_alarm');
		$host_status_label = "not Connected";
	}
}
...
sub updateCnxMIDS2 {
	if( $useMIDS ) {
		...
		if( $cnx == 1 ) {
			...
			$stopButton[$MIDS_CNX_NUM]->configure(-image => 'status_alarm');
			$mids_status_label = "not Connected";
		}
	}
	else {
		$stopButton[$MIDS_CNX_NUM]->configure(-image => 'status_alarm');
...
sub isMIDSPresent{
	...
	my $result = `plink $putty_session \"ping $mids_db -c 1 \"`;
...
sub isJREPConnected {
	my $jrep_connected = 0;
	foreach my $cnx (@jre_gateway_connexion_list){
		if ( $cnx =~ /:$jrep_port_num\s+ESTABLISHED/){
			$jrep_connected = 1;
			#last;	
		}
	}
...
sub isHOSTConnected {
	my $host_connected = 0;
	foreach my $cnx (@jre_gateway_connexion_list){
		if ( $cnx =~ /:$host_port_num\s+\s+ESTABLISHED/){
			$host_connected = 1;
			#last;	
		}
	}
...
sub saveLog {
	if(confirmAction("Save Log File ? It will stop all applications !")){
		...
		if (-d  "$LOG_DIR\\$newDir"){
			...
			# retrieve DLIP log
				if($$task->{"OPERATING_SYST"} eq "LINUX" && confirmAction("Save $task log ?")) {
...
sub clearLog{
	...
	system("del /Q E:\\THALES\\TOPLINKCOM\\log\\*");
...
sub updateContextFile {
	print "pscp jfacc_op\@$putty_session:$HOME_DIR/Ops/TDL_ROUTER/Context_Data.xml $HOME_WIN_DIR\\TOPLINKCOM\\properties \n" if( $debug);
	system("pscp jfacc_op\@$putty_session:$HOME_DIR/Ops/TDL_ROUTER/Context_Data.xml $HOME_WIN_DIR\\TOPLINKCOM\\properties\\");
}
...
sub timer2 {
	my $interval = shift;
	repeat ($interval, sub { updateStatusAll2()});
}
