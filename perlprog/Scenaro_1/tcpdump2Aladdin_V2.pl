...
my $OutputDir = ".\\";
my $OutputFile = "T_DAMB_CAPACITY_V2";
...
my $SIMPLE_IP_Address = "200.1.18.16";
my $SIMPLE_TCP_Port = "9002";

my $C4_SEND_IP_Address = "200.1.18.52";
...
sub extract2Aladdin {
	...
	open FinSIMPLEJI, ">$OutputDir$OutputFile.SIMPLE.jo" or die "Impossible ouvrir ...";
	open FoutSIMPLEJI, ">$OutputDir$OutputFile.SIMPLE.ji" or die "Impossible ouvrir ...";
	...
	#print fout "Host_Input_File=$OutputDir$OutputFile.xhd\n";
	#print fout "Host_Output_File=$OutputDir$OutputFile.xdh\n";
	print fout "FOM_Output_File = $OutputDir$OutputFile.SIMPLE.fom\n";
	print fout "FIM_Input_File = $OutputDir$OutputFile.SIMPLE.fim\n";
	close fout;
			...
			if( hex($ip_proto) == 6) {
				...
				if ($data_length_in_char != 0) {
					...
					if($ip_src eq $XHD_IP_Address && $tcp_port_src eq $XHD_TCP_Port){
						...
						# Si l'on a trouvé le mot de synchro et qu'on n'est pas dans le 1er paquet
						...
						# Si la longueur est égale et l'on a trouvé le mot de synchro 
					...
					# Le paquet TCP est un paquet SIMPLE
					...
					if(($ip_dest eq $SIMPLE_IP_Address && $tcp_port_dest eq $SIMPLE_TCP_Port)||($ip_src eq $SIMPLE_IP_Address && $tcp_port_src eq $SIMPLE_TCP_Port)) {
						...
						if($tcp_data =~/^4936(..)/ && $new_SIMPLE == 1){
							...
							#<> if($debug == 6);
							# Rajout d'une boucle pour traiter plusieurs messages dans une trame 
							# on suppose que les messages ne sont pas decoupes en plusieurs trames !
							while ((length($SIMPLE) >0) && ($SIMPLE_length_in_char <= length($SIMPLE))){
								# extraction du message SIMPLE de la trame dans $SIMPLE_msg
								my $SIMPLE_msg = substr($SIMPLE, 0, $SIMPLE_length_in_char);
								my $r_JI = SimpleMsg::new(\$SIMPLE_msg);
								SimpleMsg::printSimpleMsg();
								$SIMPLE_msg =~ /^(.{4})(..)(.{6})/;
								...
								if ($ip_dest eq $SIMPLE_IP_Address && $tcp_port_dest eq $SIMPLE_TCP_Port){
									#print "FOM : $chrono,\n$FIM, \n $JI,\n $ip_dest, $tcp_port_dest\n";
									#<>;
									# Si la reference retournee n est pas nulle alors c est un msg SIMPLE de type 1 L16
									if($r_JI != 0){
										UJ_Msg::setSTN(SimpleMsg::getSTN());
										UJ_Msg::setNPG(SimpleMsg::getNPG_number());
										UJ_Msg::setWordCount(SimpleMsg::getWordCount());
										UJ_Msg::changeEndianess($r_JI);
										UJ_Msg::formatUniversalJI($r_JI);
										print FoutSIMPLEJI "$heure:$minute:$seconde.$milli $$r_JI\n" ;
									}
									else {
									...
									if($r_JI != 0){
										...
										UJ_Msg::setWordCount(SimpleMsg::getWordCount());
										UJ_Msg::changeEndianess($r_JI);
										UJ_Msg::formatUniversalJO($r_JI);
										#print "FIM : $chrono,\n$FIM, \n $FOM,\n $ip_dest, $tcp_port_dest\n";
										#<>;
										print FinSIMPLEJI "$heure:$minute:$seconde.$milli $$r_JI\n";
									}
									else{
										print FinSIMPLE "$heure:$minute:$seconde.$milli $length 1D000000 $SIMPLE_msg\n";
										#print "$heure:$minute:$seconde.$milli $length 1D000000 $SIMPLE_msg" if($debug == 6);
									}
								}
								...
								# on recupere le reste de la trame dans $SIMPLE
