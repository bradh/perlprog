...
if ($opt_h) { 
  print "format_xhd2boctet.pl : formate les fichiers aladdin au format suivant :\n";
  print " 00:00:00.000 XXXXXXXX YYYYYYYY ZZZZ ZZZZ ZZZZ\n";
  exit(0);
}
if( ! $opt_h ) {
	my $fichierInput = "$opt_f";
	my $fichierBak = "$opt_f.bak";
	...
	open Foutbak,">$fichierBak" or die "impossible d'ouvrir le fichier de sortie $fichierBak \n";

	print "formate $fichierInput to Boctet, please wait...\n";
	while(<Fin>){
		...
		if($LIGNE =~ /^(\d\d:\d\d:\d\d\.\d\d\d)(.*)/){
		  ...
		  $LIGNE =~ s/\s(\S*)\s/$1/g;
		  #print "$LIGNE\n";
		  $LIGNE =~ s/(\S\S\S\S\S\S\S\S)(\S\S\S\S\S\S\S\S)(\S*)/$3/;
		  my $LENGTH = $1;
		  my $MSG_ID = $2;
		#print "$LIGNE\n";
		  $LIGNE =~ s/(\S\S\S\S)/ $1/g;
