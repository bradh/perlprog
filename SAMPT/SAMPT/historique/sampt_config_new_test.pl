...
# lien vers les fichiers de conf des test driver dans ATP
	system("cp -f ../ATP/host_test_driver.conf $REP_TEST_ATR/")if( -l $REP_TEST_ATR/host_test_driver.conf); 
	system("cp -f ../ATP/l16_test_driver.conf $REP_TEST_ATR/")if( -l $REP_TEST_ATR/l16_test_driver.conf); 
	system("cp -f ../ATP/ti_test_driver_c2.conf $REP_TEST_ATR/")if( -l $REP_TEST_ATR/ti_test_driver_c2.conf);
# lien vers les fichiers d'entrée
	system("cp -f ../ATP/$NOM_TEST_MINUS.xhd $REP_TEST_ATR/") if( -l $REP_TEST_ATR/$NOM_TEST_MINUS.xhd);
	system("cp -f ../ATP/$NOM_TEST_MINUS.fom $REP_TEST_ATR/") if( -l $REP_TEST_ATR/$NOM_TEST_MINUS.fom);
	system("cp -f ../ATP/$NOM_TEST_MINUS.ji $REP_TEST_ATR/") if( -l $REP_TEST_ATR/$NOM_TEST_MINUS.ji);
