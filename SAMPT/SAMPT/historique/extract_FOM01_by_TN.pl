...
if ($opt_h) { 
  print "extract_TN_date_from_fom.pl -f nom_fichier -n nber TN: extrait";
  exit(0);
}
if( ! $opt_h && $opt_f && $opt_n) {
	my $fichierInput = "$opt_f";
	my $TN_ref = $opt_n;

	open Fin, "<$fichierInput" or die "impossible d'ouvrir le fichier d'entree $fichierInput \n";
	open Fout, ">$Fichierout" or die "impossible ouvrier $Fichierout\n";
	print " Extract J with TN = $TN_ref from $fichierInput, please wait...\n";
	#for $I (0..15) {
	...
	while(<Fin>){
		...
		if($Label == 10 && $Sublabel == 2){
		  # simulation d'un TN = 12345
			print "$MOT[0] 12345 J10_2_IN $Label $Sublabel \n";
		}
		if($Label == 9 && $Sublabel == 0){
		  # simulaltion d'un TN = 12346
			print "$MOT[0] 12346 J9_0_IN $Label - $Sublabel IN \n";
		}
		if($Label == 3 && ($Sublabel == 0 || $Sublabel == 2 ||$Sublabel == 3 || $Sublabel == 5 )){
			my $TN_bit_offset = 19;
			my $TN_bit_number= 19;
			my $TN = getJValue($TN_bit_offset, $TN_bit_number) ;
			#print "$TN\n";
			print "$MOT[0] TN=$TN J$Label.$Sublabel \n"if($TN == $TN_ref);
			print Fout "$LIGNE\n"if($TN == $TN_ref);
		}
		...
		if($Label == 2) {
			my $STN = hex("$MOT[4]");
			print "$MOT[0] STN=$STN J$Label.$Sublabel\n"if($STN == $TN_ref);
		}
		if($Label == 7 && ($Sublabel == 0 || $Sublabel == 1)){
			my $TN_bit_offset = 19;
			my $TN_bit_number= 19;
			my $TN = getJValue($TN_bit_offset, $TN_bit_number) ;
			#print "TN : $String_value\n";
