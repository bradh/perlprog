package aladdin_FIM01_J3_0;

my $test = 0;

my $FIM01_J3_0 = "00:00:00.000 00000016 06000001 0000 0348 0000 0000 0000 0000 0007 0015 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000";
my $FIM01_J3_0_v6 = "00:00:00.000 00000071 00000065 0000 0000 0065 0600 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 00FF FFFF 01FF 01FF 01FF FF00 3F64 0000 0000 0000 0000 00";
my $FIM01_ref;
...
my $LTN_offset = 147;
my $LTN_length = 19;
my $relatedTN2_offset = 336;
my $relatedTN2_length = 19;
...
sub format_FIM01_J3_0{
	my $r_FIM01_J3_0 = shift;
	if($$r_FIM01_J3_0 =~ /^(\d{2}:\d{2}:\d{2}\.\d{3}) (\w{8}) (\w{8}) (.*$)/){
		my $time = $1;
		my $length = $2;
		my $msg_ID = $3;
		my $msg_data = $4;
		$msg_data =~ s/(\S{4})/$1 /g;
		$$r_FIM01_J3_0 = "$time $length $msg_ID $msg_data";
	}
}

sub get_FIM01_J3_0{
	my $r_FIM01_J3_0 = shift;
	my $message = "$r_FIM01_J3_0->{time} $r_FIM01_J3_0->{length} $r_FIM01_J3_0->{msg_id} $r_FIM01_J3_0->{msg_data}";
	print "$message\n";
	return $message;
}
...
sub setTimetag_Message{
	...
	return -1 if(!defined($r_FIM01_J3_0));
	if($$r_FIM01_J3_0 =~ /^(\d{2}:\d{2}:\d{2}\.\d{3}) (\w{8}) (\w{8}) (.*$)/){
		#print "$timetag_msg, $$r_FIM01_J3_0\n";
		my $time = $1;
		my $length = $2;
		my $msg_ID = $3;
		my $msg_data = $4;
...
sub setSTN {
	...
	#print "$STN_binary\n";
	return -1 if(! defined($r_FIM01_J3_0));
	my $binary_msg = toBinaryDataMsg($r_FIM01_J3_0, $r_FIM01_J3_0->{'msg_data'});
	#print "$binary_msg\n";
	substr($binary_msg, $STN_offset, $STN_length, $STN_binary);
	#print "$binary_msg\n";
	my $hexa_msg = binaryToHexaDataMsg($r_FIM01_J3_0, \$binary_msg);
	#print "$hexa_msg\n";
	$hexa_msg =~ s/(\S{4})/$1 /g;
	$r_FIM01_J3_0->{'msg_data'} = $hexa_msg;
	#print "$hexa_msg\n";
	return $hexa_msg;
}
...
sub addTime(){
	my $r_FIM01_J3_0 = shift;
	my $time = shift;
	$r_FIM01_J3_0->{time} =~ s/^(\d{2}:\d{2}:\d{2}\.\d{3})/$time/;
	#print "+time $$r_FIM01_J3_0\n";
}
...
sub setRelatedTN2 {
	...
	my @words_inv;
	# conversion du champs en string binaire
	my $relatedTN2_binary = sprintf("%0${relatedTN2_length}b", $relatedTN2);
	#print "$relatedTN2_binary\n";
	return -1 if(! defined($r_FIM01_J3_0));
	my $binary_msg = toBinaryDataMsg($r_FIM01_J3_0, $r_FIM01_J3_0->{'msg_data'});
	#print "$binary_msg\n";
	# inversion des mots de 16 bit
	...
	#print "words@words\n";
	my $binary_msg_inverse = join("", @words);
	my $var = -($relatedTN2_offset+ $relatedTN2_length);
	my $var2 = length($binary_msg_inverse);
	print "$var2 $var $relatedTN2_length\n";
	substr($binary_msg_inverse, -($relatedTN2_offset+ $relatedTN2_length), $relatedTN2_length, $relatedTN2_binary);
	# inversion des mots de 16 bit
	while(my $word = substr($binary_msg_inverse,0, 16, "")){
		#print "$word\n";
		push @words_inv, $word;		
...
if($test == 1){
	...
	open Fout, ">test.xhd";
	for my $i (0..90){
		$FIM01_J3_0 = $FIM01_J3_0_ref;
		setLatitude(Conversion::latitude2BAM_32($i+0.5), \$FIM01_J3_0);
		setLongitude(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_0);
		setCourse(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_0);
		#print Fout "$FIM01_J3_0\n";		
	}
	for my $i (90..180){
		$FIM01_J3_0 = $FIM01_J3_0_ref;
		setLatitude(Conversion::latitude2BAM_32(90), \$FIM01_J3_0);
		setLongitude(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_0);
		setCourse(Conversion::longitude2BAM_32($i+0.5), \$FIM01_J3_0);
		#print Fout "$FIM01_J3_0\n";		
	}
	for my $i (0..90){
		$FIM01_J3_0 = $FIM01_J3_0_ref;
		setLatitude(Conversion::latitude2BAM_32(-90+0.5), \$FIM01_J3_0);
		setLongitude(Conversion::longitude2BAM_32(-180+$i+0.5), \$FIM01_J3_0);
		setCourse(Conversion::longitude2BAM_32(-180+$i+0.5), \$FIM01_J3_0);
		#print Fout "$FIM01_J3_0\n";		
	}
	for my $i (90..180){
		$FIM01_J3_0 = $FIM01_J3_0_ref;
