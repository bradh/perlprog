 ...
 # suppression du fichier contextData lors de l'arret des applications
 # V1R2E1
 # suppression du fichier DLIP_Context a l'arret du TDL router pour un redemarrage correct du ToplinkCom
...
# Simplifaction de la commande de detection de deconnexion du MIDS
# La pop-up demandant a l'operateur si le MIDS est utilise n'est demande qu'audemarrage et au restart
...
# Z. Aslan le 30/01/15
# Modification des fonctions saveRecording et saveLog : corriger le blocage de l'IHM lors du transfert des RCD
# Creation de la fonction clearRecording
...
my $Disk_Label = "%S001%";
my $HOME_DIR="%S007%";
my $HOME_WIN_DIR="$Disk_Label\\%S002%";
my $CONFIG_DIR = "$Disk_Label\\%S003%";
my $RECORDING_DIR = "$Disk_Label\\%S004%";
my $LOG_DIR = "$Disk_Label\\%S005%";
my $putty_session = "%S007%";
my $user = "%S008%";
...
my $jrep_port_num = '%P02%';

# Supervision cnx HOST
my $host_port_num = '%P18%';
...
sub updateStatusAll {
	...
	# Si le MIDS est present et que la connexion au terminal est OFF et le MIDS est utilise et le TDL router tourne
...
sub saveRecording {
	if(confirmAction("Save Recording File ?") eq "yes" ){
		...
		if (-d  "$RECORDING_DIR"){
			...
			$remote_recording_dir = "/jfacclog/JREP";
...
sub clearRecording() {
	my $remote_recording_dir = "/jfacclog/TDL_ROUTER";
	my $cmd = "rm -fr $remote_recording_dir/*.rcd";
	system ("plink $putty_session $cmd");
		...
		if (-d  "$LOG_DIR"){
			...
			foreach my $task (@TaskList) {
				if($$task->{'OPERATING_SYST'} eq "LINUX" && confirmAction("Save $task log ?") eq "yes" ){
					...
					chdir ("..");
					my $cmd = "rm -fr $remote_log_dir/*.log";
					system ("plink $putty_session $cmd");
				}
			}
		}
