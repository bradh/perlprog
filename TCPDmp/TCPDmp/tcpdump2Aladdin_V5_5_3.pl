...
#   - Gestion des messages découpés sur 2 packets différents pour XHD
# Modif YPE le 11/05/16 (vers. 5_4_5)
#   - Amélioration de la gestion des messages découpés sur 2 packets différents: 
#     les messages ayant leurs champs Id et Taille_msg splités sur 2 packets differents sont gérés
...
if (1) {
	...
	$mw->new_ttk__label(-text => "C3M interface : ", -width => 16)
		->g_grid(-column => 5, -row => 4, -sticky => "w");
	$mw->new_ttk__checkbutton( -variable => \$C3M_config, -command => \&radiobuttonproceed, -width => 1)
		->g_grid(-column => 6, -row => 4, -sticky => "we");
                    ...
                    # J-P. Coron, le 20 octobre 2009 - Fin modification		
                      ...
                      for my $i (0..$data_length_in_byte-1){
                        ...
                        # recherche du premier octet de synchro
                         if($etatLectureXDH == 0) {
                            if(($char =~/44/ && !$C3M_config) || ($char =~ /00/ && $C3M_config) ){
                            #if($char =~ /00/){
                              $etatLectureXDH = 1;
                              #$XDH_msg = $char;
...
# Le paquet TCP est un XHD					
					if($ip_dest eq $XHD_IP_Address && $tcp_port_dest eq $XHD_TCP_Port){
						...
						while ($length > 0){
							...
							if(($tcp_data =~/^4844(....)/ && $new_XHD == 1 && !$C3M_config) || ($tcp_data =~/^0000(....)/ && $new_XHD == 1 && $C3M_config) ){
