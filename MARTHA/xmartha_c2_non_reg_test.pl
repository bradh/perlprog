...
my $ButtonStart=$frame2->new_ttk__button(-text=>'Start Non Reg', -state => 'active', -command => [\&startThread, $lb] , , -width => 12)
...
my $KOButton = $frame2->new_ttk__button(-command => \&startTread,
...
$lb->insert(0, "Test insid toto : OK");
$lb->insert(0, "Test insid titi : OK");

my $timer1 = threads->create(\&timer, \&timer_test, 1, 5, $lb);
$lb->insert(0, "Test insid toto : OK");
$lb->insert(0, "Test insid titi : OK");
...
sub startThread {
	...
	my $th = threads->create(\&startNonReg, $lb);
	#my $toto = $th->join();
...
sub timer {
my($subroutine, $interval, $max_iteration, $lb) = @_;
...
sub writeInTheBox {
	$lb->insert(0, "Test in the box : OK");
}