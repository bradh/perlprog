...
J2_2_DELTA_TIME=24  # durée pendant laquelle sont émis les J
                    # chaque message est émis 1 fois
                    # delta_time = REC_RATE 
J2_2_DELTA_NBR=32   # nbre de réccurrence de chaque J
...
J2_3_DELTA_TIME=24  # durée pendant laquelle sont émis les J
                     # chaque message est émis 1 fois
                     # delta_time = REC_RATE 
J2_3_DELTA_NBR=32    # nbre de réccurrence de chaque J
################################################
# Parametres  J2.5
################################################
...
J2_5_DELTA_TIME=24  # durée pendant laquelle sont émis les J
                      # chaque message est émis 1 fois
                      # delta_time = REC_RATE 
J2_5_DELTA_NBR=32    # nbre de réccurrence de chaque J
################################################
# Parametres  J3.0
################################################
J3_0_NBR=20          # 10
...
J3_2_DELTA_TIME=24  # durée pendant laquelle sont émis les J
                      # chaque message est émis 1 fois
                      # delta_time = REC_RATE 
J3_2_DELTA_NBR=32    # nbre de réccurrence de chaque J
################################################
# Parametres  J3.6
################################################
J3_6_NBR=3          #
...
J3_6_DELTA_TIME=24  # durée pendant laquelle sont émis les J
                     # chaque message est émis 1 fois
                     # delta_time = REC_RATE 
J3_6_DELTA_NBR=32    # nbre de réccurrence de chaque J
################################################
# Parametres  J3.5
################################################
J3_5_NBR=1          #
...
J3_7_DELTA_TIME=96  # durée pendant laquelle sont émis les J
                    # chaque message est émis 1 fois
                    # delta_time = REC_RATE 
J3_7_DELTA_NBR=8    # nbre de réccurrence de chaque J
...
################################################
# Make originator (J2.2(TN=1)) of J9.0 messages
################################################

export source_tn_1=1
./insert_value_into_ji_header.sh dec 15 32 $source_tn_1 "00:00:00.000""$J2_2" >> $TMP_OUTPUT_FILE

################################################
# Make periodic sequence for J9.0 :6 on J3.2 for 45 minutes
################################################
...
################################################
# Make periodic sequence for J2.2 :30 in 12 sec
...
################################################
# Make periodic sequence for J2.3 :50 in 12 sec
#  (warning 30 of these J2.2 will be J2.3 when using SAMPT exe)
################################################
...
################################################
# Make periodic sequence for J2.5 :30 in 12 sec
################################################
...
################################################
# Make periodic sequence for J3.0 :60 in 12min48 sec
################################################

clear_tmp_file
export tn_reference=513
export sce_time=0 
export compt=0
while ([[ $compt -ne $J3_0_NBR ]]) ;do 
  export index_j3_0=`echo|awk '{print '$compt'%4+1}'`
  #register only main tn of line area
  # because to drop a line area , one must
  # only J7.0 drop  on one of its TN
  if [[ $index_j3_0 -eq 1 ]] then
     register_tn_created $tn_reference
  fi
  export J3_0=`extract_from_templates 3 0 $index_j3_0`
  ./insert_value_into_ji.sh dec 19 19 $tn_reference $sce_time"$J3_0" |awk -f ./to_hour_minute_second.awk >> $TMP_OUTPUT_FILE
  export tn_reference=`expr $tn_reference + 1`;
  export compt=`expr $compt + 1`;
  export sce_time=`echo |awk '{print '$compt'%4+4*int('$compt'/4)*'$J3_0_REC_RATE'/'$J3_0_NBR'}'`;
done

################################################
# Make periodic sequence for J3.2 :242 in 12 sec
...
################################################
# Make periodic sequence for J3.6 :242 in 12 sec
#  (warning 2 of these 3.2 will be J3.6 when using SAMPT exe)
################################################
...
################################################
# Make periodic sequence for J3.5 :20 in 96 sec
################################################
...
################################################
# Make periodic sequence for J3.7 :12 in 48 sec
################################################
